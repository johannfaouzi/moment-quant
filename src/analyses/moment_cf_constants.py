...
from src.utils import APPROX_INTERVALS, APPROX_SAMPLES, DEPTH, DIV

estimators = [APPROX_SAMPLES, APPROX_INTERVALS]
...
def basis_for_length(depth, div, length):
    ...
    return w_tilde, nq_tilde, ni_tilde
...
def main():
    ...
    print("=" * 100)
    print(f"{'L':>6} {'Wtilde':>14} {'NQtilde':>10} {'NItilde':>10} {'Wtilde/NQtilde':>16}")
    for L in L_values:
        w, nq, ni = basis[L]
        ratio = w / nq if nq else float("nan")
        print(f"{L:6d} {w:14.2f} {nq:10.0f} {ni:10.0f} {ratio:16.4f}")

    A = np.array([basis[L] for L in L_values])
    print()
    print(f"Design matrix condition number (Wtilde, NQtilde, NItilde): {np.linalg.cond(A):.4g}")
    print("Correlation matrix (Wtilde, NQtilde, NItilde):")
    print(np.corrcoef(A.T))
    print()
    print("A high condition number / near-1 correlation between Wtilde and NQtilde means c~_1 and")
    print("c~_2 are only weakly separable from each other in the joint fit below (their combination")
    print("is much more stable than either individually) -- see the 'c1_plus_ratio_c2_tilde' row")
    print("for a more robust combined number, and treat the individual c~_1/c~_2 rows with")
    print("appropriate caution. c~_3 (NItilde's coefficient) is expected to stay comparatively well")
    print("identified, since NItilde saturates to a depth-dependent constant for large L while")
    print("Wtilde/NQtilde keep growing -- a genuinely different shape.")
    ...
    for estimator in estimators:
        ...
        c1_tilde, c2_tilde, c3_tilde = coeffs
        pred = A_est @ coeffs
        ...
        c1_robust, c3_robust = coeffs_reduced
        ...
        for name, value in [("c1_tilde", c1_tilde), ("c2_tilde", c2_tilde), ("c3_tilde", c3_tilde)]:
            results.append(dict(Estimator=estimator, Constant=name, Value=value, R2=r2_full, Fit="joint_3param"))
        results.append(
            dict(
                Estimator=estimator,
                Constant="c1_plus_ratio_c2_tilde",
                Value=c1_robust,
                R2=r2_reduced,
                Fit="robust_2param",
            )
        ...
        results.append(
            dict(Estimator=estimator, Constant="c3_tilde", Value=c3_robust, R2=r2_reduced, Fit="robust_2param")
        )

    out = pd.DataFrame(results, columns=["Estimator", "Constant", "Value", "R2", "Fit"])
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "moment_cf_constants.csv")
