"""One-to-one NumPy translation of quant_original.py, but using double precision (float64)."""
...
def f_quantile(X, div=4):
    ...
    if n == 1:
        ...
        if num_quantiles == 1:
            quantiles = X.quantile(torch.tensor([0.5], dtype=torch.float64), dim=-1).permute(1, 2, 0)
            return quantiles.view(quantiles.shape[0], 1, quantiles.shape[1] * quantiles.shape[2])

        else:
            quantiles = X.quantile(torch.linspace(0, 1, num_quantiles, dtype=torch.float64), dim=-1).permute(1, 2, 0)
